// Panel + modal trang Environment (hạ tầng đô thị): quản lý vận hành, PCCC,
// công trình, cảnh báo hạ tầng, đường ống. Tách từ smartcity-domain-command-panels.js.
import {
...
export function infrastructureHealthSnapshot({ total, gaugePct, metrics }) {
...
export function environmentNetwork({ slices: sliceData, items }) {
  let angle = -22;
...
export function environmentRadar({ bars, status }) {
...
export function pcccFireRiskNetwork({ nodes: nodeData }) {
...
export function environmentLoadMatrix({ items }) {
  const cols = items;
...
export function constructionBuildingsPanel({ sites }) {
...
export function infrastructureAlertsPanel({ axes, items }) {
...
export function pipeInfrastructurePanel({ stabilityValue, stabilityLabel, trend, kpis }) {
