...
function addMountains(scene) {
  ...
  const mat = new THREE.MeshStandardMaterial({ color: 0x7a9cb8, roughness: 0.95, fog: false });
  for (let i = 0; i < 12; i++) {
...
function addCitySilhouette(scene) {
  ...
  const mat = new THREE.MeshStandardMaterial({ color: 0x8898a8, roughness: 0.9, fog: false });
  for (let i = 0; i < 24; i++) {
